...
TEST_CASE("getting the current prayer") {
  ...
  CHECK(
      p.currentPrayer(dateByAddingSeconds(p.fajr, -1).value()) == Prayer::None);
  CHECK(p.currentPrayer(p.fajr.value()) == Prayer::Fajr);
  CHECK(
      p.currentPrayer(dateByAddingSeconds(p.fajr, 1).value()) == Prayer::Fajr);
  CHECK(
      p.currentPrayer(dateByAddingSeconds(p.sunrise, 1).value()) ==
      Prayer::Sunrise);
  CHECK(
      p.currentPrayer(dateByAddingSeconds(p.dhuhr, 1).value()) ==
      Prayer::Dhuhr);
  CHECK(p.currentPrayer(dateByAddingSeconds(p.asr, 1).value()) == Prayer::Asr);
  CHECK(
      p.currentPrayer(dateByAddingSeconds(p.maghrib, 1).value()) ==
      Prayer::Maghrib);
  CHECK(
      p.currentPrayer(dateByAddingSeconds(p.isha, 1).value()) == Prayer::Isha);
}
...
TEST_CASE("getting the next prayer") {
  ...
  CHECK(
      p.nextPrayer(dateByAddingSeconds(p.fajr, 1).value()) == Prayer::Sunrise);
  CHECK(
      p.nextPrayer(dateByAddingSeconds(p.sunrise, 1).value()) == Prayer::Dhuhr);
  ...
  CHECK(
      p.nextPrayer(dateByAddingSeconds(p.maghrib, 1).value()) == Prayer::Isha);
